...
public class MemberMyInfoActivity extends AppCompatActivity {
	...
	public void onChangeBirth(final View view) {
		...
		Matcher m = p.matcher(currentBirth);
		if (m.find()) {
		...
		new DatePickerDialog(MemberMyInfoActivity.this, new DatePickerDialog.OnDateSetListener() {

			@Override
			public void onDateSet(DatePicker datePicker, int year, int month, int day) {
				String date = "%4d-%02d-%02d";
	...
	public void onClickApply(final View view) {
		...
		final MemberInfoItem newMemberInfoItem = currentMemberInfoItem.clone();
		...
		call.enqueue(new Callback<String>() {
			...
			public void onResponse(Call<String> call, Response<String> response) {
				...
				if (response.isSuccessful()) {
					...
					((MyApp)getApplication()).CurrentMemberInfo = newMemberInfoItem;
